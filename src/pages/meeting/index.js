...
Page({
   ...
   */
  async onReady() {
    const { number, password, displayName, videoMute, audioMute } =
      this.pageOption
  ...
  secondToDate(result) {
    ...
    const h = formatVal(Math.floor(result / 3600))
    const m = formatVal(Math.floor((result / 60) % 60))
    const s = formatVal(Math.floor(result % 60))
